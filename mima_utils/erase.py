...
if __name__ == '__main__':
    ...
    if concepts[0] == 'allartist':
        concepts = ["Kelly Mckernan", "Thomas Kinkade", "Kirbi Fagan", "Tyler Edlin", "Kilian Eng", "Van Gogh", "Claude Monet", "Pablo Picasso"]
    if concepts[0] == 'i2g':
    ...
    if preserve_concepts is None:
        if concept_type == 'art':
            df = pd.read_csv('data/prompts/artists1734_prompts.csv')
    ...
    save_path = f"results/{args.concepts}/"
    os.makedirs(save_path, exist_ok=True)
    torch.save(ldm_stable.unet.state_dict(), f'{save_path}/delta_ckpt.pt')
    print(f'Saved model at {save_path}/delta_ckpt.pt')
