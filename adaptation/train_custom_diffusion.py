...
sys.path.append("./")
from mima_utils.adaptation_utils import log_validation, load_imma_pro_ckpt
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The trained imma-pro ckpt added to the loaded pretrained model"
        ),
    )
    parser.add_argument(
        "--pretrained_model_name_or_path",
    ...
    parser.add_argument(
        "--loaded_concepts_list",
        type=ast.literal_eval,
        default=None,
        help="list of loaded concepts",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    if args.with_prior_preservation:
        if args.concepts_list is None and args.loaded_concepts_list is None:
...
def main(args):
    ...
    if args.seed is not None:
        set_seed(args.seed)
    if args.loaded_concepts_list is not None:
        args.concepts_list = args.loaded_concepts_list
        args.validation_prompt = args.concepts_list[0]["validation_prompt"]
    else:
    ...
    # load imma pro attention weights to the pre-trained unet
    if args.imma_pro_ckpt is not None:
        try:
            load_imma_pro_ckpt(unet, args.imma_pro_ckpt)
        except:
            if not args.imma_pro_ckpt or args.imma_pro_ckpt == "false" or args.imma_pro_ckpt == "None":
                print("No imma-pro ckpt provided, using the default unet weights.")
                pass
            else:
                print("Failed to load imma-pro ckpt {}, please check the path and try again.".format(args.imma_pro_ckpt))
                raise
